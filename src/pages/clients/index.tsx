...
import { createInvoiceData, getInvoiceData } from 'src/store/apps/invoice'
import DataGridTable from 'src/components/Datagrid'
import Modal from 'src/components/Model/Model'
import InvoiceModal from 'src/components/InvoiceModal'
import RecieptList from 'src/components/RecieptList'
...
const ClientComponent = () => {
  ...
  const [totalWithVat, setTotalWithVat] = useState<any>(0);
  ...
  const handleListInvoice = async (id: string, row: any) => {
  ...
  const handleCloseInvoiceListModal = () => {
  ...
  const handleInvoiceSubmit = async () => {
    ...
    if (!invoiceData.discount || !invoiceData.amountPaid) {
      ...
      await dispatch(updateLeadData({ data: { client: true }, where: { id: selectedLead.id } }))
  ...
  return (
    <Grid container spacing={6}>
      ...
      <RecieptList
        ...
        handleInvoiceListClose={handleCloseInvoiceListModal}
        data={invoiceList}
      />
